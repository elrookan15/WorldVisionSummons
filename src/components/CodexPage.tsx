...
import { codexStyleById } from "../lib/codexStyles";
import type { CodexSnapshot } from "../lib/codex/finalizer";

type CodexPageProps = {
  snapshot: CodexSnapshot;
  plateRef?: Ref<HTMLElement>;
...
export default function CodexPage({ snapshot, plateRef, portraitUrl, onPortraitError }: CodexPageProps) {
  const page = compose(snapshot);
  const sourcePortrait = portraitUrl === undefined ? snapshot.assets.portrait.url : portraitUrl;
  const displayName = page.name || "Unnamed Summon";
  const leftCallouts = page.abilities.slice(0, 3);
  const rightCallouts = page.callouts;
  const skin = codexStyleById(snapshot.styleId);
  const locked = snapshot.snapshotId !== "draft";
  const skinVars = {
...
<div className="codex-stage">
        <article
          ...
          data-page-size={snapshot.pageSize}
          ...
          <div className="codex-frame">
            ...
            <div className="codex-body">
              ...
              <div className="codex-portrait-well" data-codex-zone="portrait">
                {sourcePortrait ? (
                  <img src={sourcePortrait} alt="" data-codex-portrait="live" onError={onPortraitError} />
                ) : (
