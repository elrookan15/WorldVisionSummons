...
export default function DiceTray({ sheet, c, fonts }: DiceTrayProps) {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-40 no-print" style={{ fontFamily: fonts.body }}>
      {open && (
        <div
          className="mb-3 w-[min(100vw-2.5rem,22rem)] rounded-2xl border shadow-2xl overflow-hidden"
          style={{ backgroundColor: c.card, borderColor: c.border, color: c.text, boxShadow: `0 18px 50px ${c.shadow}` }}
        >
          <div className="px-4 py-3 border-b flex items-center justify-between" style={{ borderColor: c.border, backgroundColor: c.bg2 }}>
            <div>
              <div className="mono text-[9px] tracking-[0.16em] uppercase" style={{ color: c.muted }}>Tabletop Tray</div>
          ...
          <div className="p-4 space-y-3">
              ...
              {(["normal", "advantage", "disadvantage"] as DiceMode[]).map((m) => (
                <button
                  ...
                  style={{
                    ...
                    borderColor: mode === m ? c.borderStrong : c.border
                  }}
                >
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-1.5">
                {sheet.stats.map((st) => {
                  ...
                  return (
                    ...
                    >
                      <span>{st.key}</span>
                      <span className="mono" style={{ color: c.accent }}>{formatModifier(mod)}</span>
                    </button>
                  );
                })}
              </div>
              <button
                ...
                style={{ backgroundColor: c.bg2, borderColor: c.borderStrong, color: c.text }}
              >
            ...
            {last && (
              <div
                ...
                style={{
                  ...
                  borderColor: last.natural === "crit" ? "#10b981" : last.natural === "fail" ? "#e11d48" : c.border
                }}
              >
