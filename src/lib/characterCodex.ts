...
function cloneSheet(sheet: UiSheetData): UiSheetData {
  return JSON.parse(JSON.stringify(sheet)) as UiSheetData;
}
...
function summarize(entry: Omit<CodexEntry, "sheetData"> & { sheetData: UiSheetData }): CodexEntry {
  const sheet = cloneSheet(entry.sheetData);
  return {
    ...entry,
    name: sheet.name || entry.name || "Unnamed",
    title: sheet.title || entry.title || "",
    classRole: sheet.overview?.classRole || entry.classRole || "",
    sheetStyle: sheet.sheet_style || entry.sheetStyle || "Gothic Dark Fantasy",
...
export function persistCodex(entries: CodexEntry[]): { ok: boolean; droppedPortraits: boolean } {
  ...
  try {
    ...
    const withoutPortraits = trimmed.map((e) => ({
      ...e,
      portraitUrl: e.portraitUrl?.startsWith("data:") ? null : e.portraitUrl
    }));
    try {
...
export function duplicateCodexEntry(id: string): { entries: CodexEntry[]; entry: CodexEntry | null } {
  ...
  const now = new Date().toISOString();
  const copiedSheet = cloneSheet(source.sheetData);
  copiedSheet.name = `${copiedSheet.name} (Copy)`;
  const copy = summarize({
    ...source,
    id: uuidv4(),
    savedAt: now,
    updatedAt: now,
    name: copiedSheet.name,
    sheetData: copiedSheet
  });
